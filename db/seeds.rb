...
child_category.each do |parent_child|
  # parent_childの名前が親カテゴリの名前であるか比較
  if parent_child == parent_category[parent_id_cnt]
    ...
    # Child.new(parent_child,parent_id_cnt)
    child_array << [parent_child,parent_id_cnt ]
  end
end
# # 最後にターミナルに完成した配列を出力。
# # p "親カテゴリ:#{parent_category}"
# # p "子カテゴリ:#{child_array}" 

parent_id_cnt=0
grandchild_array=[]
grand_child_category.each do |child_grandchild|
  # parent_childの名前が親カテゴリの名前であるか比較
  if child_grandchild == child_array[parent_id_cnt][0]
    ...
    # Grand_child.new(child_grandchild,parent_id_cnt + 13)
...
grandchild_array.each do |grandchild,num|
